...
void interpolate(const uint8_t width, const uint8_t height,
    ...
    if (X(0) < 0 || X(1) < 0)
        return; // coordenadas dos centros não podem ser negativas
    ...
    log_v("r^2 %f", r);
    if (r <= 0)
        return; // previne contra raiz de número negativo
    r = sqrt(r);
    log_v("r %f", r);
    if (r > (width / 2) || r > (height / 2))
        return; // raio não pode ser maior que metade do comprimento da imagem
